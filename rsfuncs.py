...
def gdf_to_ee_poly(gdf, simple = True):
...
def get_croptype(ee_image, area):
	cdl = ee.Image(ee_image)
	polys = gen_polys(area)
	d = polys.getInfo()
...
def load_data():
	...
	data['prism_daily'] = [ee.ImageCollection("OREGONSTATE/PRISM/AN81d"), "ppt", 1, 800]
	...
	data['l8_sr'] = [ee.ImageCollection("LANDSAT/LC08/C01/T1_SR"), "B1", 0.0001	, 30]
	data['l7_sr'] = [ee.ImageCollection("LANDSAT/LE07/C01/T1_SR"), "B1", 0.0001	, 30]

	#########################
	##### RADAR data ########
	#########################
	data['s1'] = [ee.ImageCollection('COPERNICUS/S1_GRD'), 'VV', 1, 10]
